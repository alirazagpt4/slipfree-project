...
export default function App() {
  ...
  useEffect(() => {
    async function loadReceipt() {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/receipts/${hash}`);
  ...
  async function handleFeedback(label: string) {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/receipts/${hash}/feedback`, {
        method: 'POST',
